...
problem. It does not treat that data as an autonomous legal-decision problem.
This distinction matters for agent-facing systems in any jurisdiction. The same
event stream that helps a reviewer find missing evidence could be misused. This
can happen if a model treats candidate events as certified statutory outcomes.
The repository therefore keeps assertion status, provenance, human review, and
