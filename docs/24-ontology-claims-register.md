...
| FOI-O is a global model that originated in NZ and has iterated through Australian jurisdictions. | Supported as an architecture and governance claim. | `README.md`, `docs/39-ontology-versioning-and-jurisdiction-profiles.md`, this register | Global does not mean universally legally validated. NZ remains the mature reference implementation; AU-CTH and AU-NSW maturity is bounded to their pinned ontology and evidence scopes. |
...
| FOI-O has iterated beyond NZ through Australian jurisdictions. | Supported by bounded AU-CTH/AU-NSW contract, source, annotation, reliability, and extractor evidence. | Australian adapter contracts, source-pack approvals, maturity decisions, and profile registries. | Do not describe bounded extractor maturity as legal validation, gold promotion, population-wide inference, or unrestricted release. |
