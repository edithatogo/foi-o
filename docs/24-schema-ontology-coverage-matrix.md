...
| Extraction contract | `schemas/json/extraction-contract.schema.json`, `contracts/foi-o-extraction-contract/0.1.0/manifest.json` | Pins `ontology/foi-o-nz.ttl`, the ontology-release manifest schema, and the candidate assertion-status vocabulary. | `examples/v2/schema-valid/extraction-contract-1.json` plus three rejection fixtures | `tests/test_extraction_contract.py`, `tests/test_empirical_schema_fixtures.py` | `nlp-policy-nz` outputs remain candidate-only; certification and promotion require human approval. |
