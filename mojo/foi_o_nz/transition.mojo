...
def transition_rank(state: String) -> Int:
    if state == "Drafted":
        return 0
    if state == "Submitted":
        return 1
    ...
    if state == "DocumentsIdentified" or state == "ConsultationRequired":
        return 8
    if (
        state == "ThirdPartyConsultation"
        or state == "ChargeAssessment"
        or state == "ExtensionApplied"
    ):
        return 9
    ...
    if state == "DecisionApproved":
        return 12
    if (
        state == "ReleasedInFull"
        or state == "ReleasedInPart"
        or state == "Refused"
        or state == "NoDocumentsFound"
        or state == "Withdrawn"
        or state == "Closed"
    ):
        return 13
...
def is_forward_transition(from_state: String, to_state: String) -> Bool:
