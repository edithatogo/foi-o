...
def build_agent_context_pack(
    *,
    ...
    material = {
        ...
        "request_hash": sha256_text(canonical_record_json(request)) if request else None,
        "events_hash": _hash_records(events),
        "chunks_hash": _hash_records(chunks),
        "risks_hash": _hash_records(risks),
    }
    pack_digest = sha256_text(canonical_record_json(material))[:24]
    constraints = {
        "agent_may": [
            "retrieve_context",
    ...
    return AgentPack(
        ...
        provenance={
            ...
            "events_jsonl": str(events_jsonl) if events_jsonl else None,
