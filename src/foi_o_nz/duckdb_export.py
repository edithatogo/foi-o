...
def build_duckdb_database(
    *,
    ...
    if requests_parquet is not None:
        con.execute(
            "CREATE OR REPLACE TABLE requests AS SELECT * FROM read_parquet(?)",
            [str(requests_parquet)],
        )
        tables.append("requests")
    elif requests_jsonl is not None:
        con.execute(
            "CREATE OR REPLACE TABLE requests AS SELECT * FROM read_json_auto(?)",
            [str(requests_jsonl)],
        )
        tables.append("requests")
    if events_parquet is not None:
        con.execute(
            "CREATE OR REPLACE TABLE events AS SELECT * FROM read_parquet(?)", [str(events_parquet)]
        )
