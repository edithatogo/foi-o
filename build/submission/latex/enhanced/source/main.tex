...
information as a core part of accountable government {[}9,12,13{]}.
...
remain responsible for legally meaningful decisions {[}22-24{]}.
...
avoids unsupported claims about live deployment readiness {[}25{]}.
...
validation problem. It does not treat that data as an autonomous
legal-decision problem. This distinction matters for agent-facing
systems in any jurisdiction. The same event stream that helps a reviewer
find missing evidence could be misused. This can happen if a model
treats candidate events as certified statutory outcomes. The repository
therefore keeps assertion status, provenance, human review, and
...
publication duties, and reporting rules. A reusable ontology cannot
assume that every jurisdiction shares New Zealand terms or deadlines. It
can, however, provide a common pattern for the evidence trail. That
trail includes requests, observed correspondence, candidate events,
...
case. It gives the project a concrete starting point. The purpose is to
make later comparative work easier {[}1-13{]}.
...
judgement remains with authorised people {[}22-24{]}.
...
simple local checks and richer ontology work can coexist {[}14-19{]}.
...
inspection, and reproducible validation {[}20,21,25{]}. The contribution
