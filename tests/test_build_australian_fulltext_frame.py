...
def test_fulltext_frame_preserves_capture_disposition(tmp_path: Path) -> None:
    ...
    artifact.write_text(json.dumps({"schema": "fyi-archive.historical-fulltext.v1", "records": [
        {"source_url": "https://example.test/request/a?utm_source=x", "status": "captured", "text": "A", "text_sha256": "a" * 64},
        {"source_url": "https://example.test/request/b", "status": "failed", "diagnostic": "403", "text": ""},
    ]}), encoding="utf-8")
    ...
    assert frame["units"][0]["source_ref"]["canonical_source_url"] == "https://example.test/request/a?utm_source=x"
