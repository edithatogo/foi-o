...
def test_import_isolation() -> None:
    ...
    for name, module in list(sys.modules.items()):
        if name.startswith("foi_o_nz.oia_rules."):
            ...
            assert "models" not in imports or name.endswith(
                "types"
            )  # types may import models for clock reference if needed, but rules does not.
...
def test_fixtures_conformance() -> None:
    ...
    if not fixtures_path.exists():
        fixtures_path = (
            Path(__file__).parent.parent / "rules" / "fixtures" / "oia-clock-fixtures.json"
        )
    ...
    holidays_file = (
        Path(__file__).parent.parent / "examples" / "nz-public-holidays-2026.govt-nz.json"
    )
    if not holidays_file.exists():
        holidays_file = (
            ...
            / "foi-o"
            / "examples"
            / "nz-public-holidays-2026.govt-nz.json"
        )
    holidays = set()
    if holidays_file.exists():
        with holidays_file.open(encoding="utf-8") as hf:
            hdata = json.load(hf)
    ...
    for case in fixtures["cases"]:
        ...
        for dec_id, expected_out in case["expected"].items():
            ...
            # Verify outputs
            assert actual_out.valueState == expected_out.get("valueState", "known"), (
                f"Mismatch in {case['caseId']} status"
            )
            if expected_out.get("value") is not None:
                assert actual_out.value == expected_out.get("value"), (
                    f"Mismatch in {case['caseId']} value"
                )
            if expected_out.get("warnings") is not None:
                for w in expected_out["warnings"]:
                    ...
                    assert any(w in aw for aw in (actual_out.warnings or [])), (
                        f"Expected warning '{w}' missing in {case['caseId']}"
                    )
