...
def test_exact_batched_execution_approval_is_plan_bound_and_downstream_inert() -> None:
    ...
    assert sha256(approval["approval_statement"].encode("utf-8")).hexdigest() == approval["approval_statement_sha256"]
    ...
    for key in ("empirical_result_approved", "human_reviewed", "gold_eligible", "publication_allowed", "external_action_allowed"):
