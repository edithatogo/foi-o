...
- [ ] Create a committed request packet for the exact RightToKnow scope,
      selected instance(s), page/runtime caps, source endpoint, private output
      location, and explicit `EXPORT_ALL_CAPTURE_METADATA` authorization.
- [ ] Run the manually confirmed CDX export. Accept it only when the evidence
...
| --- | --- | --- |
| Exact all-captures request | Pending human authorization | Scope, caps, endpoint, private retention path, confirmation token |
